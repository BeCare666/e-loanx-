...
window.addEventListener("DOMContentLoaded", () => {
  ...
  someOtherFunction(steps);
});
/**
 * Fonction externe qui utilise l'instance de StepIndicator
 * @param {StepIndicator} stepIndicatorInstance - L'instance de StepIndicator
 */

class StepIndicator {
  /** Element utilisé pour cet indicateur de progression */
  el = null;
  /** Nombre d'étapes */
  steps = 5;
  ...
  set step(value) {
      ...
      this._step = value;
      this.checkExtremes();
      // Enregistrer l'étape actuelle dans le localStorage
      localStorage.setItem("currentStep", value);
     
  }
  /**
   * @param el Sélecteur CSS de l'élément indicateur d'étape
   */
  constructor(el) {
      this.el = document.querySelector(el);

      // Récupérer l'étape depuis le localStorage si elle existe
      const savedStep = localStorage.getItem("currentStep");
       
      if (savedStep !== null) {
          this._step = parseInt(savedStep, 10);
      }
  ...
  /** Désactiver le bouton Précédent ou Suivant si on atteint la première ou la dernière étape. */
  checkExtremes() {
  ...
  /**
   * Mettre à jour l'indicateur pour une étape ciblée.
   * @param targetStep Index de l'étape
   */
